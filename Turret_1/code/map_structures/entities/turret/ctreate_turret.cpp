...
std::unique_ptr<Turret> Turret::createTurret(const int type, const TileCoord tile)
{
	switch (type)
	{
	case AUTOCANNON_TURRET:
		return std::make_unique<AutocannonTurret>(AUTOCANNON_TURRET, tile);
	case ROCKET_TURRET:
		return std::make_unique<RocketTurret>(ROCKET_TURRET, tile);
	default:
		return nullptr;
	}
}
